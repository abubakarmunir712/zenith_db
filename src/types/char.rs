...
impl CHAR {
    ...
    pub fn to_bytes(&self, allow_padding: bool) -> Vec<u8> {
        ...
        // Add the length of the string (2 bytes for the length)
        result.extend_from_slice(&(self.value.len() as u16).to_le_bytes());
    ...
    pub fn from_bytes(bytes: Vec<u8>, size: u32) -> Result<Self, CharError> {
        ...
        let value =
            String::from_utf8(bytes[2..2 + length].to_vec()).map_err(|_| CharError::InvalidUtf8)?;
